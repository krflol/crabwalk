...
| Non-executing source embedding | Bounded | content-addressed source compilation and direct native callable binding | native top-level side-effect rejection-by-nonexecution and callable test<br>Contracts: `embedding.nonexecuting-source-callable`, `embedding.phase-cancellation` | single-module source snapshots; Cargo dependencies remain a trusted build boundary; cancellation is phase-cooperative |
...
| `Vec[T]` | `push`, `pop`, `reserve`, `len`, `is_empty`, `iter`, `iter_ref`, consuming `into_iter`; typed `par_iter` with declared Rayon; numeric teaching intrinsic `split_at_mut_sum` |
